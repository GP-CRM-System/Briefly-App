...
const MessageThread = ({ 
    ...
    if (!messages.length) {
        ...
        const renderMessageContent = (msg: Message, isInbound: boolean) => {
        ...
        if (msg.type === "image") {
            return (
                <div className="relative max-w-xs mt-1 rounded-xl overflow-hidden shadow-xs border border-gray-200 bg-black/5 animate-fade-in">
                    <img
                        src={previewUrl}
                        alt="Attachment"
        ...
        if (msg.type === "audio") {
            return (
                <div className={`flex flex-col gap-2 p-3 rounded-xl w-64 mt-1 border shadow-xs relative animate-fade-in ${
                    isInbound 
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className={`text-xs font-semibold truncate ${isInbound ? "text-slate-800" : "text-white"}`}>{metadata?.originalName || "Voice Record"}</div>
                            <div className={`text-[10px] mt-0.5 ${isInbound ? "text-slate-400" : "text-white/60"}`}>{formatBytes(metadata?.size)}</div>
                        </div>
                    </div>
                    <audio src={previewUrl} controls className="w-full mt-1 accent-blue-500" style={{ height: '32px' }} />
                    {isUploading && (
                        <div className="absolute inset-0 bg-black/65 backdrop-blur-xs flex items-center justify-between px-4 rounded-xl animate-fade-in">
        ...
        if (msg.type === "document") {
            ...
            return (
                <div className={`flex items-center gap-3 p-2.5 rounded-xl max-w-xs mt-1 border relative animate-fade-in ${
                    isInbound 
                        ? "bg-slate-50 border-slate-100 text-slate-800" 
                        : "bg-white/10 border-white/10 text-white"
                }`}>
                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 font-bold text-[10px] uppercase tracking-wider ${
                        isInbound 
                    ...
                    <div className="min-w-0 flex-1">
                        <div className={`text-xs font-semibold truncate ${isInbound ? "text-slate-800" : "text-white"}`}>{metadata?.originalName || "Attachment"}</div>
                        <div className={`text-[10px] mt-0.5 ${isInbound ? "text-slate-400" : "text-white/60"}`}>{formatBytes(metadata?.size)}</div>
                    </div>
                    {!isUploading ? (
                        <a
        ...
        if (msg.type === "sticker") {
            return (
                <div className="relative max-w-[120px] mt-1 rounded-xl overflow-hidden bg-transparent animate-fade-in">
                    <img
                        src={previewUrl}
                        alt="Sticker"
            ...
            {Object.entries(groupedMessages).map(([dateStr, dayMessages]) => (
                <div key={dateStr} className="space-y-3">
                    ...
                    <div className="flex justify-center my-4">
                        <span className="bg-white/80 backdrop-blur-xs text-gray-500 text-[10px] font-bold px-3 py-1 rounded-full shadow-xs border border-gray-100 uppercase tracking-wider animate-fade-in">
                    ...
                    {dayMessages.map((msg) => {
                        ...
                        return (
                            <div
                                key={msg.id}
                                className={`flex ${isInbound ? "justify-start pl-2" : "justify-end pr-2"}`}
                                    ...
                                    <div
                                        className={
                                            msg.type === "sticker"
                                                ...
                                                : `rounded-2xl px-4 py-2.5 shadow-2xs ${
                                                    isInbound
                                                        ? "bg-white text-gray-900 rounded-tl-sm border border-slate-150"
                                                        : isFailed
                                                          ? "bg-red-50 text-red-900 border border-red-150 rounded-tr-sm"
                                                          : "bg-[var(--color-primary-500)] text-white rounded-tr-sm"
                                                }`
                                        }
                                    >
                                        {renderMessageContent(msg, isInbound)}
                                    ...
                                    {msg.type === "sticker" && (
                                        <div className={`flex items-center gap-1.5 mt-1 ${isInbound ? "justify-start" : "justify-end"}`}>
                                            ...
                                            {!isInbound && (
                                                <span className="flex items-center">
                                                    {msg.status === "SENT" && (
                                                        <svg className="h-3.5 w-3.5 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                                                            <path d="M20 6L9 17l-5-5" />
                                                        </svg>
                                                    )}
                                                    {msg.status === "DELIVERED" && (
                                                        <svg className="h-3.5 w-4.5 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                                                            <path d="M17 6L8.5 14.5L5 11" />
                                                            <path d="M22 6l-8.5 8.5L12 13" />
                                                        </svg>
                                                    )}
                                                    {msg.status === "READ" && (
                                                        <svg className="h-3.5 w-4.5 text-blue-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                                                            <path d="M17 6L8.5 14.5L5 11" />
                                                            <path d="M22 6l-8.5 8.5L12 13" />
                                                        </svg>
                                                    )}
