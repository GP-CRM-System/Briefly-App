...
const AcceptInvitation = () => {
    ...
    const acceptInvite = useCallback(async (id: string) => {
        ...
        try {
            ...
            // Set the joined organization as the active one for the current session.
            // Best-effort: if this fails, the invitation is still accepted.
            try {
                ...
                // Non-critical — invitation is already accepted
            }
    ...
    }, [invitationId, isAuthenticated, acceptInvite]);

    // Save invitation ID before redirecting to login so it can be picked up after sign in
    ...
    const initialValues = { name: '', email: prefilledEmail, password: '' };
        ...
        return (
            <>
                ...
                <p className="text-center text-sm text-gray-600 mt-6">
                    ...
                    <Link to={`/login?invitationId=${invitationId}`} onClick={handleLoginRedirect} className="font-semibold text-[var(--color-primary-500)] underline ml-1">
                        Sign in
                    </Link>
                </p>
            </>
