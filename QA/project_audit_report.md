...
*   **Main Differentiators:**
    ...
    *   **Pre-Deletion Protection:** Anti-data-loss constraint where an organization cannot be deleted without first executing a full tenant data export, triggering automated admin notification emails.
    *   **Multi-Instance Real-Time Scalability:** WebSocket server using a Redis adapter to synchronize state across multiple server nodes, combined with BullMQ persistent background queues for resilient data pipelines.
...
*   **Permissions:** Nearly identical to Root, excluding organization deletion. Has `organization:read/update`, `member:read/create/update/delete`, `invitation:read/create/cancel`, `team:read/create/update/delete`, `ac:read`, and full read/write/delete capabilities on business features (Customers, Orders, Payments, Products, Imports, Exports, Integrations, Webhooks, Sync, Segments, Campaigns, Support Tickets, Tags, Reports, Notifications, Templates, Conversations, Subscriptions).
...
*   **Permissions:** Restricted read/write access. Permissions: `customers:read`, `orders:read`, `products:read`, `exports:read`, `integrations:read`, `segments:read`, `campaigns:read`, `supportTickets:read/write`, `tags:read`, `notifications:read`, `templates:read`, `conversations:read`, `subscriptions:read`.
...
1.  **Multi-Tenant Signup & Onboarding:**
...
3.  **Shopify Custom OAuth & Settings:**
...
4.  **Shopify Auto-Sync Pipeline:**
    ...
    *   *Why it exists:* To synchronise storefront data into the CRM without triggering rate-limit blocks.
    *   *User Roles:* Root, Admin.
    *   *Status:* Completed.
    *   *Frontend Pages:* `/dashboard/settings` (Connections Tab)
...
5.  **Shopify Storefront Web Pixel Ingestion:**
    ...
    *   *Status:* Completed.
    *   *Frontend Pages:* Storefront integration script.
...
6.  **RFM Scoring Engine:**
    ...
    *   *APIs:* `/api/cron/rfm`
7.  **Customer Lifecycle & Churn Risk Tracking:**
...
8.  **Customer Profile Timeline & Activity Logs:**
...
9.  **Social Messaging Inbox:**
    ...
    *   *APIs:* `/api/messaging/*`
10. **Rich Media Attachments & S3 Upload Session:**
...
12. **Dynamic Segment Builder:**
    ...
    *   *APIs:* `/api/segments/*`
13. **Marketing Campaigns & Templates:**
...
14. **Paymob Subscription Gateway:**
...
Strict database tenancy is enforced because all business models (`Customer`, `Order`, `Product`, `Segment`, `Campaign`, `SupportTicket`, `Conversation`, `Integration`, `ImportJob`, `ExportJob`, `AuditLog`) maintain a direct, indexed foreign key to the `Organization` table. 

Join queries across different tenant scopes are blocked at the database level by composite indexes, such as `@@unique([organizationId, name])` on the `Segment` table and `@@unique([organizationId, externalId])` on the `Conversation` table.
...
```
[Register/Login] ➔ Better Auth Route ➔ Prisma Adapter ➔ Cookie Cache / JWT
                                                                ↓
[API Request] ➔ headers / cookies ➔ protect Middleware ➔ requirePermission Guard ➔ Handler
```

*   **Login Flow:** The frontend sends email/password payloads or Google OAuth redirects to the Better Auth endpoints. Better Auth verifies credentials, initializes session state, writes to the `Session` table, and sets cookies.
*   **Registration Flow:** User registers via `/signup`. Better Auth creates a new user, checks the organization limit (max 1), creates an organization record, assigns the creator as the `root` member, and invokes a database hook to automatically assign a free tier subscription plan (`assignFreePlanToOrg`).
*   **Session Management:** Sessions are tracked in the database (`Session` model) and cached in cookies. The session expires in 7 days (`expiresIn: 60 * 60 * 24 * 7`) with a 1-day rolling update window.
*   **JWT & Token Usage:** In addition to cookie authentication, the system supports header token verification (`Bearer` plugin) for API-first requests.
*   **RBAC & Permissions:** Implemented via Better Auth's organization access control (`createAccessControl`). The system defines three roles:
...
```
[Meta App Webhook] ➔ Express webhook Router ➔ verifySignature ➔ Queue Webhook Job (BullMQ)
                                                                            │
   [Socket.io Broadcast] ◄─── emitToOrg ◄─── handleInboundMessage ◄─────────┘
```
...
*   **Real-time Features:** Real-time updates are handled via Socket.io with a Redis adapter for multi-instance sync. It handles online presence tracking, typing indicators, and upload progress.
...
```
[UI Composer] ➔ POST /upload-session ➔ Verify Extension ➔ optimistic Message & B2 Key
                                                                      │
[Complete Msg] ◄── POST /complete-upload ◄── Upload File ◄── Presigned Upload URL
```

1.  **Request Session:** The client requests an upload session by passing the file name, mime type, size, and category (image, video, audio, or document).
2.  **Validation:** The system validates the file extension against an allowed list:
    *   `image`: jpg, jpeg, png, webp, gif
    *   `video`: mp4, mov, webm
    *   `audio`: mp3, wav, m4a, ogg
    *   `document`: pdf, doc, docx, xls, xlsx, ppt, pptx, txt, zip, rar, csv
3.  **Placeholder Record:** The system creates a placeholder message in the database with a `PENDING` status.
4.  **Presigned Upload URL:** The system generates a B2 storage key:
    `chat-[type]s/org_[orgId]/conv_[convId]/msg_[msgId]/[uuid].[ext]`
    It returns a presigned upload URL from Backblaze B2.
5.  **Direct Upload:** The client uploads the file directly to Backblaze B2.
6.  **Complete Upload:** The client calls `/complete-upload`. The system verifies the upload, fetches a pre-signed download URL, updates the message content to the download URL, changes the status to `SENT`, and queues the outbound message job.
...
*   `POST /api/auth/signup`
    ...
    *   *Auth Required:* No.
    *   *Request Body:* `email`, `password`, `name`, `organizationName`.
    *   *Response:* User session payload.
*   `POST /api/auth/login`
    *   *Purpose:* Authenticate user.
    *   *Auth Required:* No.
    *   *Request Body:* `email`, `password`.
    *   *Response:* Session cookies and user payload.
*   `GET /api/me`
    *   *Purpose:* Fetch active session details.
    *   *Auth Required:* Yes.
    *   *Response:* Current user, active organization, role, and permissions.

### Customers Module
*   `GET /api/customers`
    *   *Purpose:* Paginated list of customer profiles.
    *   *Auth Required:* Yes (`customers:read`).
    *   *Response:* List of customers, total count.
*   `POST /api/customers`
    *   *Purpose:* Create a customer profile.
    *   *Auth Required:* Yes (`customers:write`).
    *   *Request Body:* `name`, `email`, `phone`, `city`, `address`, `source`, `lifecycleStage`.
    *   *Response:* Created customer record.
*   `GET /api/customers/:id/timeline`
    *   *Purpose:* Fetch customer timeline events.
    *   *Auth Required:* Yes (`customers:read`).
    *   *Response:* Chronological feed of orders, notes, and tickets.

### Unified Messaging Module
*   `GET /api/messaging/conversations`
    *   *Purpose:* Paginated list of organization conversations.
    *   *Auth Required:* Yes (`conversations:read`).
    *   *Response:* Conversations array, total counts.
*   `POST /api/messaging/conversations/:id/messages`
    *   *Purpose:* Send outbound message.
    *   *Auth Required:* Yes (`conversations:write`).
    *   *Request Body:* `content`, `type`, `metadata`.
    *   *Response:* Queued message.
*   `POST /api/messaging/conversations/:id/messages/upload-session`
    *   *Purpose:* Create media upload session.
    *   *Auth Required:* Yes (`conversations:write`).
    *   *Request Body:* `fileName`, `mimeType`, `fileSize`, `type`.
    *   *Response:* Presigned B2 upload URL, placeholder message.

### Subscriptions Module
*   `POST /api/subscriptions/paymob/initialize`
    *   *Purpose:* Create subscription payment intention.
    *   *Auth Required:* Yes (`subscriptions:write`).
    *   *Request Body:* `planId`, `billingCycle`.
    *   *Response:* Paymob payment redirection URL.
*   `POST /api/subscriptions/paymob/callback`
    *   *Purpose:* Paymob payment callback.
    *   *Auth Required:* Checksum Signature verified.
...
*   **Hosting:**
    *   *Frontend:* Vercel.
    *   *Backend:* Node.js on Render or AWS ECS.
...
```
User Signup ➔ Create Org ➔ Get Free Tier Plan ➔ Connect Shopify ➔ Auto-Sync Triggered
```
...
```
Storefront Event ➔ Ingest Web Pixel ➔ Recalculate RFM ➔ Stage Transition ➔ Churn Notification
```
...
1.  **Meta Webhook Broker:** Handled using Zod validation, HMAC signatures, BullMQ queues, and Socket.io broadcasts.
2.  **Storefront Web Pixel Resolver:** Matches storefront pixel payloads to existing CRM customer records.
3.  **Redis-Adapter Socket.io Server:** Synchronizes WebSocket events across multiple server nodes.
4.  **BullMQ Queue Failover:** Automatically switches to inline synchronous processing if Redis goes offline.
5.  **Paymob Intention Payments:** Manages Paymob billing flows, signatures, and callbacks.
6.  **Pre-Deletion Protection Hook:** Blocks organization deletion until data is exported and uploaded to B2.
7.  **Shopify REST Link-Header Sync:** Syncs large merchant datasets using link-header pagination.
8.  **AES-256-GCM Encryption:** Encrypts and decrypts third-party integration tokens.
9.  **Presigned B2 Media Uploads:** optimistic message creation, B2 upload URL generation, and download URL signing.
10. **RFM Aggregator:** Computes scores in batches using PostgreSQL aggregate functions.
11. **Better Auth Custom Session Adapter:** Dynamically merges role memberships and permissions into active sessions.
12. **XLSX/CSV Import Mapping:** Applies dynamic column mappings to database structures.
13. **PDF Invoice Generator:** Generates PDFs dynamically using `pdfkit-table`.
14. **Dynamic Segment SQL Compiler:** Parses filter rules and translates them into Prisma queries.
15. **Campaign Link Tracker:** Modifies HTML campaign links to track clicks, opens, and bounces.
16. **VIP Percentile Processor:** Evaluates and updates the top 5% of spenders in transactions.
17. **Webhook Idempotency Layer:** Uses TTL verification records to deduplicate incoming webhooks.
18. **Better Auth Invite Workflow:** Manages secure, link-based email invitations.
19. **Sentry Context Logger:** Tracks and logs exceptions with transaction contexts.
20. **Zustand Presence Store:** Tracks online user states using Socket presence events.
